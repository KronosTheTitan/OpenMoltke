...
bool RefractionShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"OpenMoltke/shaders/refraction.vs");
	if(error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"OpenMoltke/shaders/refraction.ps");
	if(error != 0)
	{
		return false;
	}
...
bool RefractionShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, reinterpret_cast<LPCSTR>(vsFilename), "Missing Shader File", MB_OK);
		}
	...
	if(FAILED(result))
	{
		...
		{
			MessageBox(hwnd, reinterpret_cast<LPCSTR>(psFilename), "Missing Shader File", MB_OK);
		}
